...
export async function sendPush(userId:string,title:string,body:string,url="/"){
  ...
  await Promise.all(rows.map(async row=>{try{await webpush.sendNotification(JSON.parse(row.subscription),JSON.stringify({title,body,url}),{TTL:3600})}catch(error){const status=(error as {statusCode?:number}).statusCode;if(status===404||status===410)await db.delete(pushSubscriptions).where(eq(pushSubscriptions.id,row.id))}}));
}
