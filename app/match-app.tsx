...
export default function MatchApp({displayName,authProvider,authContact,preview=false}:{displayName:string;authProvider:string;authContact:string;preview?:boolean}){
  ...
  const saveProfile=async(event:FormEvent<HTMLFormElement>)=>{
    ...
    if(!profile.trainerName.trim()||profile.mainPokemon.length===0||!profile.gender||!profile.contact.trim()){notify("必須項目を入力してください");return}
  ...
  const shareTrainerCard=async()=>{
    ...
    ctx.font="700 25px sans-serif";ctx.fillText(profile.playTime,112,545);
