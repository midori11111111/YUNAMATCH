...
type AppTab = "discover" | "recruit" | "chat" | "community" | "profile";
...
export default function MatchApp({displayName,authProvider,authContact,preview=false}:{displayName:string;authProvider:string;authContact:string;preview?:boolean}){
  ...
  const submitRecruit=async(event:FormEvent<HTMLFormElement>)=>{
    ...
    if(preview){const recruit={...previewRecruit,id:-2,trainerName:profile.trainerName,gender:profile.gender||"女性",pokemon:String(body.pokemon),role:String(body.role),matches:Number(body.matches),winRate:Number(body.winRate),rank:profile.highestRate,playTime:profile.playTime.join("・"),note:typeof body.note==="string"?body.note.trim():""};setMyRecruit(recruit);setRecruitShare(recruit);setCompose(false);setTab("recruit");notify("募集を公開しました");return}
    setSending(true);
  ...
  const recruitShareText=(recruit:Recruit)=>[`【ポケモンユナイト仲間募集】`,`${recruit.pokemon} / ${recruit.role}`,`${recruit.rank}・勝率${recruit.winRate}%`,recruit.playTime,recruit.note,"#YUNAMATCH #ポケモンユナイト募集"].filter(Boolean).join("\n");
  ...
  const saveProfile=async(event:FormEvent<HTMLFormElement>)=>{
    ...
    if(!profile.trainerName.trim()||profile.mainPokemon.length===0||profile.playTime.length===0||!profile.gender||!profile.contact.trim()){notify("必須項目を入力してください");return}
  ...
  const shareTrainerCard=async()=>{
    ...
    ctx.font="700 25px sans-serif";ctx.fillText(profile.playTime.join(" / "),112,545);
    const pokemonImagePath=getPokemonImagePath(primaryPokemon);let imagePainted=false;
    if(pokemonImagePath){try{const image=await new Promise<HTMLImageElement>((resolve,reject)=>{const value=new Image();value.onload=()=>resolve(value);value.onerror=reject;value.src=pokemonImagePath});const scale=Math.min(340/image.naturalWidth,390/image.naturalHeight);const width=image.naturalWidth*scale;const height=image.naturalHeight*scale;ctx.save();ctx.shadowColor="#24164e88";ctx.shadowBlur=28;ctx.shadowOffsetY=18;ctx.drawImage(image,790+(340-width)/2,150+(390-height)/2,width,height);ctx.restore();imagePainted=true}catch{/* 画像の読み込み失敗時は文字を表示 */}}
    if(!imagePainted){ctx.textAlign="right";ctx.font="900 172px sans-serif";ctx.fillText(primaryPokemon.slice(0,1),1065,465);ctx.textAlign="left"}
        ...
        {loading?<div className="stateCard"><div className="loadingBall"/><h2>メイトを探しています</h2></div>:current?<>
          <article className={`discoverCard ${animation}`} onPointerDown={event=>setDragStart(event.clientX)} onPointerUp={handlePointerUp}>
            ...
            <div className="cardDetails">
              ...
              <div className="timeChip"><span>◷</span><div><small>遊べる時間帯</small><strong>{current.playTime}</strong></div></div>{current.note&&<p className="profileNote"><strong>ひとこと</strong>“{current.note}”</p>}
            </div>
          </article>
  ...
  {compose&&<div className="modalBackdrop"><form className="sheetModal formSheet" onSubmit={submitRecruit}><button type="button" className="closeButton" onClick={()=>setCompose(false)}>×</button><small className="modalKicker">CREATE RECRUIT</small><h2>メイトを募集</h2><div className="twoFields"><label>使用ポケモン<select name="pokemon" defaultValue={primaryPokemon}>{profile.mainPokemon.map(name=><option key={name}>{name}</option>)}</select></label><label>型<select name="role"><option>アタック型</option><option>バランス型</option><option>スピード型</option><option>ディフェンス型</option><option>サポート型</option></select></label></div><div className="twoFields"><label>試合数<input name="matches" type="number" min="0" max="99999" defaultValue="1000" required/></label><label>勝率<input name="winRate" type="number" min="0" max="100" step="0.1" defaultValue="50" required/></label></div><label>ひとこと <small>任意</small><textarea name="note" maxLength={180} placeholder="未入力でも募集できます"/></label><p className="privacyText">{profile.trainerName}・{profile.highestRate}・{profile.playTime.join("・")}で募集します。連絡先は承認した相手にだけ表示されます。</p><button className="primaryButton" disabled={sending}>{sending?"公開中…":"募集を公開する"}</button></form></div>}
