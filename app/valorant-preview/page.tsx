  ...
  [
    {
      ...
      name: "Riot Games",
      mark: "R",
      note: "戦績・ランクを本人確認付きで表示",
    },
    {
      id: "discord",
      name: "Discord",
      mark: "D",
      note: "VCや普段使うアカウントで続ける",
    },
    { id: "x", name: "X", mark: "𝕏", note: "Xのアカウントで続ける" },
    { id: "line", name: "LINE", mark: "L", note: "LINEのアカウントで続ける" },
  ];
const tabs: { id: Tab; icon: string; label: string }[] = [
  { id: "discover", icon: "⌕", label: "さがす" },
  { id: "recruit", icon: "+", label: "募集" },
...
export default function ValorantPreviewPage() {
  ...
  const load = async () => {
    const [d, c, r] = await Promise.all([
        fetch("/api/services/valomatch/discover"),
        fetch("/api/services/valomatch/connections"),
        fetch("/api/services/valomatch/recruits"),
      ]),
      [dd, cc, rr] = await Promise.all([d.json(), c.json(), r.json()]);
  ...
  useEffect(() => {
    let live = true;
    fetch("/api/services/valomatch/profile")
      .then(async (response) => {
  ...
  async function like() {
    if (!current) return;
    const response = await fetch("/api/services/valomatch/likes", {
        method: "POST",
  ...
  async function requestMate() {
    if (!current) return;
    const response = await fetch("/api/services/valomatch/connections", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ targetProfileId: current.id }),
      }),
  ...
  async function createRecruit() {
    const mode = prompt(
      "モード（コンペティティブ / アンレート / スイフトプレイ / その他）",
      "コンペティティブ",
    );
    if (!mode) return;
    const partySize = Number(prompt("パーティ人数（2〜5）", "5")),
      note = prompt("募集のひとこと（任意）", "") || "";
    const response = await fetch("/api/services/valomatch/recruits", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          mode,
  ...
  async function openChat(connection: Connection) {
    ...
    const response = await fetch(
        `/api/services/valomatch/messages?connectionId=${connection.id}`,
      ),
      data = await response.json();
  ...
  async function sendMessage(event: FormEvent) {
    ...
    setMessage("");
    const response = await fetch("/api/services/valomatch/messages", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
    ...
    return (
      <main className={styles.loginPage}>
        <section className={styles.loginHero}>
          ...
          <p>
            ランク・ロール・時間帯から、VALORANTを一緒に遊ぶ仲間を探せます。
          </p>
        ...
        <section className={styles.loginSheet}>
          <span className={styles.sheetHandle} />
          <p className={styles.kicker}>SELECT ACCOUNT</p>
          <h2>アカウントを選んで続ける</h2>
          <p className={styles.loginCopy}>
            登録済みの方は同じアカウントを選ぶと、以前のプロフィールを引き継げます。
          </p>
          <div className={styles.providerGrid}>
            {providers.map((x) => (
              <button
                key={x.id}
                className={`${styles.provider} ${styles[x.id]}`}
  ...
  return (
    <main className={styles.appPage}>
      <header className={styles.appHeader}>
        <button className={styles.miniAvatar} onClick={() => setTab("profile")}>
          SO
        </button>
      ...
      <section className={styles.appContent}>
            ...
            {current ? (
              <article className={styles.profileCard}>
                <div className={styles.cardTop}>
                  <span className={styles.online}>● プロフィール公開中</span>
                  <b>{current.skillTier}</b>
                </div>
                <div className={styles.agentShape}>
                ...
                <div className={styles.cardInfo}>
                  ...
                  <p className={styles.bio}>
                    {current.bio || "一緒に遊べる仲間を探しています。"}
                  </p>
                  <small className={styles.riotVerified}>
                    遊べる時間：{current.playTimes.join(" · ")}
                  </small>
                </div>
              ...
              <section className={styles.simpleScreen}>
                <h1>表示できるプレイヤーがいません</h1>
                <p>時間を置いて更新してください。</p>
              </section>
            )}
          </>
        ...
        {tab === "recruit" && (
          <section className={styles.simpleScreen}>
            <p className={styles.kicker}>FIND A PARTY</p>
            <h1>今すぐ遊べる募集</h1>
            {recruits.length ? (
              recruits.map((item) => (
                <article className={styles.recruitCard} key={item.id}>
                  <div>
                    <span>{item.mode}</span>
                    <small>
            ...
            <button className={styles.createRecruit} onClick={createRecruit}>
              ＋ 募集を作成
            </button>
          </section>
        )}
        {tab === "chat" && (
          <section className={styles.simpleScreen}>
            ...
            <h1>やりとり</h1>
            {incoming.map((item) => (
              <article className={styles.matchNotice} key={item.id}>
                <span>⚡</span>
                <div>
                  <strong>{item.other.displayName}さんから申請</strong>
                  <p>
            ...
            {connections.map((item) => (
              ...
              >
                <span>{item.other.displayName.slice(0, 2)}</span>
                <div>
        ...
        {tab === "profile" && (
          <section className={styles.simpleScreen}>
            <div className={styles.profileHero}>
              <span>{profileData?.displayName.slice(0, 2)}</span>
              <p className={styles.kicker}>MY PROFILE</p>
              <h1>{profileData?.displayName}</h1>
              <p>
                {profileData?.gameIdentity} · {profileData?.skillTier} ·{" "}
      ...
      {activeChat && (
        ...
        >
          ...
          {messages.map((item) => (
            <p
              key={item.id}
              style={{ padding: 12, background: "#f1edff", borderRadius: 14 }}
            >
              {item.body}
            </p>
          ))}
