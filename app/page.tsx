...
export default async function Home({
  ...
  if (user) {
    ...
    if (!row) initialProfile = null;
      ...
      initialProfile={trainerName:row.trainerName,mainPokemon,highestRate:normalizeRank(row.highestRate),playTime,gender:row.gender==="男性"||row.gender==="女性"?row.gender:"",contact:row.contact,bio:row.bio||"",avatarUrl:row.avatarUrl||"",headerUrl:row.headerUrl||"",age:row.age,ageConfirmed:Boolean(row.ageConfirmed),termsAccepted:Boolean(row.termsAcceptedAt)};
    }
  }
