    ...
    db
      ...
      .where(
        and(
          ...
          eq(serviceLikes.recipientProfileId, ctx.profile.id),
          eq(serviceLikes.status, "active"),
        ),
      )
      .orderBy(desc(serviceLikes.createdAt))
      .limit(201),
    db
      .select({ recipientProfileId: serviceLikes.recipientProfileId })
      .from(serviceLikes)
      .where(
        and(
          eq(serviceLikes.serviceId, ctx.service),
          eq(serviceLikes.senderProfileId, ctx.profile.id),
          eq(serviceLikes.status, "active"),
        ),
      )
      .limit(1000),
...
export async function POST(
  ...
  await db
    ...
    .onConflictDoUpdate({
      target: [
        serviceLikes.serviceId,
        serviceLikes.senderProfileId,
        serviceLikes.recipientProfileId,
      ],
  ...
  const [reciprocal] = await db
    ...
    .where(
      and(
        eq(serviceLikes.serviceId, ctx.service),
        eq(serviceLikes.senderProfileId, target.id),
        eq(serviceLikes.recipientProfileId, ctx.profile.id),
        eq(serviceLikes.status, "active"),
      ),
    )
...
export async function PATCH(
  ...
  const likeId =
    typeof body.likeId === "number" && Number.isInteger(body.likeId)
      ? body.likeId
      : 0;
  ...
  const [updated] = await getDb()
    ...
    .returning();
  if (!updated)
    return Response.json({ error: "いいねが見つかりません" }, { status: 404 });
