import { and, desc, eq, gt, inArray, lt, or } from "drizzle-orm";
import { getDb } from "../../../../../db";
import {
  serviceBlocks,
  serviceProfiles,
  serviceRecruits,
} from "../../../../../db/schema";
import { getChatGPTUser } from "../../../../chatgpt-auth";
import {
  containsProhibitedContent,
  prohibitedContentMessage,
} from "../../../../../lib/content-policy";
import {
  checkRateLimit,
  rateLimitResponse,
} from "../../../../../lib/rate-limit";
import { normalizeShoenmateTier, shoenmateUsername } from "../../../../../lib/shoenmate-profile";
import {
  cleanText,
  isServiceId,
  serviceConfig,
  stringList,
} from "../../../../../lib/service-config";

async function getContext(params: Promise<{ service: string }>) {
  const { service } = await params;
  if (!isServiceId(service)) return null;
  const user = await getChatGPTUser();
  return user ? { service, user } : null;
}
function output(row: typeof serviceRecruits.$inferSelect) {
  return { ...row, desiredRoles: JSON.parse(row.desiredRoles) as string[] };
}

export async function GET(
  request: Request,
  { params }: { params: Promise<{ service: string }> },
) {
  const { service } = await params;
  if (!isServiceId(service))
    return Response.json({ error: "サービスIDが不正です" }, { status: 404 });
  const url = new URL(request.url),
    requestedId = Number(url.searchParams.get("id") || 0),
    before = Number(url.searchParams.get("before") || 0),
    now = new Date(),
    db = getDb(),
    user = await getChatGPTUser(),
    [own] = user
      ? await db
          .select({ id: serviceProfiles.id })
          .from(serviceProfiles)
          .where(
            and(
              eq(serviceProfiles.serviceId, service),
              eq(serviceProfiles.userId, user.userId),
            ),
          )
          .limit(1)
      : [],
    blockRows = own
      ? await db
          .select({
            a: serviceBlocks.blockerProfileId,
            b: serviceBlocks.blockedProfileId,
          })
          .from(serviceBlocks)
          .where(
            and(
              eq(serviceBlocks.serviceId, service),
              or(
                eq(serviceBlocks.blockerProfileId, own.id),
                eq(serviceBlocks.blockedProfileId, own.id),
              ),
            ),
          )
      : [],
    hidden = new Set(
      blockRows.map((row) => (row.a === own?.id ? row.b : row.a)),
    );
  const rows = await db
      .select()
      .from(serviceRecruits)
      .where(
        and(
          eq(serviceRecruits.serviceId, service),
          requestedId ? eq(serviceRecruits.id, requestedId) : undefined,
          eq(serviceRecruits.status, "open"),
          gt(serviceRecruits.expiresAt, now),
          before ? lt(serviceRecruits.id, before) : undefined,
        ),
      )
      .orderBy(desc(serviceRecruits.id))
      .limit(61),
    page = rows.filter((row) => !hidden.has(row.ownerProfileId)).slice(0, 30),
    ownerIds = [...new Set(page.map((row) => row.ownerProfileId))],
    owners = ownerIds.length
      ? await db
          .select()
          .from(serviceProfiles)
          .where(
            and(
              eq(serviceProfiles.serviceId, service),
              inArray(serviceProfiles.id, ownerIds),
            ),
          )
      : [],
    byId = new Map(owners.map((row) => [row.id, row]));
  return Response.json({
    recruits: page.map((row) => {
      const owner = byId.get(row.ownerProfileId);
      return {
        ...output(row),
        owner: owner
          ? {
              id: owner.id,
              displayName: service === "shoenmate" ? shoenmateUsername(owner.displayName, owner.gameIdentity) : owner.displayName,
              skillTier: service === "shoenmate" ? normalizeShoenmateTier(owner.skillTier) : owner.skillTier,
              roles: JSON.parse(owner.roles),
              characters: JSON.parse(owner.characters),
              avatarUrl: owner.avatarUrl,
              headerUrl: owner.headerUrl,
              gender: owner.showGender && owner.age >= 18 ? owner.gender : "",
              bio: owner.bio,
            }
          : null,
      };
    }),
    nextBefore:
      !requestedId && rows.length > 30
        ? (rows[Math.min(rows.length, 60) - 1]?.id ?? null)
        : null,
  });
}

export async function POST(
  request: Request,
  { params }: { params: Promise<{ service: string }> },
) {
  const ctx = await getContext(params);
  if (!ctx)
    return Response.json(
      { error: "ログインまたはサービスIDを確認してください" },
      { status: 401 },
    );
  const limit = await checkRateLimit(`${ctx.service}:${ctx.user.userId}`, {
    action: "service-recruit",
    limit: 8,
    windowMs: 30 * 60_000,
  });
  if (!limit.allowed) return rateLimitResponse(limit.retryAfter);
  const db = getDb(),
    [profile] = await db
      .select()
      .from(serviceProfiles)
      .where(
        and(
          eq(serviceProfiles.serviceId, ctx.service),
          eq(serviceProfiles.userId, ctx.user.userId),
          eq(serviceProfiles.status, "active"),
        ),
      )
      .limit(1);
  if (!profile || profile.suspendedAt)
    return Response.json(
      { error: "先にこのサービスのプロフィールを登録してください" },
      { status: 403 },
    );
  const body = (await request.json().catch(() => ({}))) as Record<
      string,
      unknown
    >,
    config = serviceConfig[ctx.service],
    mode = cleanText(body.mode, 40),
    partySize =
      typeof body.partySize === "number" && Number.isInteger(body.partySize)
        ? body.partySize
        : 0,
    desiredRoles = stringList(body.desiredRoles, 5),
    note = cleanText(body.note, 160),
    startAt =
      typeof body.startAt === "string" && body.startAt
        ? new Date(body.startAt)
        : null,
    minutes =
      typeof body.durationMinutes === "number"
        ? Math.min(1440, Math.max(15, body.durationMinutes))
        : 120;
  if (
    !config.modes.has(mode) ||
    partySize < 2 ||
    partySize > 5 ||
    desiredRoles.some((role) => !config.roles.has(role)) ||
    (startAt && Number.isNaN(startAt.getTime()))
  )
    return Response.json(
      { error: "募集条件を確認してください" },
      { status: 400 },
    );
  if (containsProhibitedContent(note))
    return Response.json({ error: prohibitedContentMessage }, { status: 400 });
  const now = new Date(),
    [row] = await db
      .insert(serviceRecruits)
      .values({
        serviceId: ctx.service,
        ownerProfileId: profile.id,
        mode,
        partySize,
        desiredRoles: JSON.stringify(desiredRoles),
        startAt,
        note,
        status: "open",
        expiresAt: new Date(now.getTime() + minutes * 60_000),
        createdAt: now,
        updatedAt: now,
      })
      .returning();
  return Response.json({ recruit: output(row) }, { status: 201 });
}
