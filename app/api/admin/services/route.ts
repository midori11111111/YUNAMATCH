      ...
      SERVICES.map(async (service) => {
        const [
          ...
          db
            ...
            .where(eq(serviceProfiles.serviceId, service.id)),
          db
            .select({ value: count() })
            .from(serviceRecruits)
            .where(eq(serviceRecruits.serviceId, service.id)),
          db
            .select({ value: count() })
            .from(serviceConnections)
            .where(eq(serviceConnections.serviceId, service.id)),
          db
            .select({ value: count() })
            .from(serviceMessages)
            .where(eq(serviceMessages.serviceId, service.id)),
          db
            .select({ value: count() })
            .from(serviceReports)
            .where(eq(serviceReports.serviceId, service.id)),
          db
            .select({ value: count() })
            .from(serviceReports)
            .where(
              sql`${serviceReports.serviceId}=${service.id} and ${serviceReports.status}<>'resolved'`,
            ),
        ]);
        return {
          ...service,
          profiles: profiles[0]?.value ?? 0,
          recruits: recruits[0]?.value ?? 0,
          connections: connections[0]?.value ?? 0,
          messages: messages[0]?.value ?? 0,
          reports: reports[0]?.value ?? 0,
