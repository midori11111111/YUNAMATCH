...
export async function GET(){
  ...
  const rows=await getDb().select({id:reports.id,targetId:reports.targetId,reason:reports.reason,details:reports.details,status:reports.status,createdAt:reports.createdAt,targetName:profiles.trainerName,avatarUrl:profiles.avatarUrl,suspendedAt:profiles.suspendedAt}).from(reports).leftJoin(profiles,eq(reports.targetId,profiles.userId)).orderBy(desc(reports.createdAt)).limit(200);
...
export async function PATCH(request:Request){
  ...
  if(payload.action==="resolve"&&payload.reportId)await db.update(reports).set({status:"resolved"}).where(eq(reports.id,payload.reportId));
