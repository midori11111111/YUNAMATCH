...
export async function POST(request:Request){
  ...
  const role=String(options.role||"指定なし"),currentRank=String(options.current_rank||"").trim(),partyChoice=String(options.party_size||"up_to_3"),partySize=partyChoice==="up_to_3"?3:Number(partyChoice),partyLabel=partyChoice==="up_to_3"?"3人以下":`${partySize}人`,startsIn=Number(options.starts_in||0),duration=Number(options.duration||2),matches=Number(options.matches||0),winRate=Number(options.win_rate||50);
  ...
  const [recruit]=await db.insert(recruits).values({ownerId:profile.userId,trainerName:profile.trainerName,gender:profile.gender,pokemon,role,matches:Math.max(0,Math.round(matches)),winRate:Math.min(100,Math.max(0,winRate)),rank:currentRank,playTime,note:partyChoice==="up_to_3"?"Discordから募集中（3人以下）":"Discordから募集中",contact:profile.contact,startAt,expiresAt,partySize,desiredPokemon:"すべて",desiredRole:"指定なし",createdAt:now}).returning();
