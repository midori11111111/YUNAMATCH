...
export async function POST(request:Request) {
  ...
  const [row]=await db.insert(recruits).values({ownerId:user.userId,trainerName:profile.trainerName,gender:profile.gender,pokemon,role,matches:Math.round(matches),winRate,rank:profile.highestRate,playTime:profilePlayTimes.filter(Boolean).join("・")||profile.playTime,note,contact:"",startAt,startTimeUndecided,expiresAt,partySize,desiredPokemon,desiredRole,createdAt:now}).returning();
