...
export async function GET(request:Request) {
  ...
  const visibleRecruit=(row:typeof rows[number])=>({id:row.id,trainerName:row.trainerName,gender:row.gender,pokemon:row.pokemon,role:row.role,matches:row.matches,winRate:row.winRate,rank:normalizeRank(row.rank),playTime:row.playTime,note:row.note,createdAt:row.createdAt,avatarUrl:row.avatarUrl||"",headerUrl:row.headerUrl||"",bio:row.bio||"",startAt:row.startAt,startTimeUndecided:row.startTimeUndecided,expiresAt:row.expiresAt,partySize:row.partySize,desiredPokemon:row.desiredPokemon,desiredRole:row.desiredRole,matchType:row.matchType,acceptedCount:row.acceptedCount});
...
export async function POST(request:Request) {
  ...
  return Response.json({recruit:{...row,avatarUrl:profile.avatarUrl,headerUrl:profile.headerUrl||"",bio:profile.bio||""}},{status:201});
}
