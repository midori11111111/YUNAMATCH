...
export async function DELETE(request:Request){
  ...
  await d1.batch([
    ...
    d1.prepare("DELETE FROM lobby_members WHERE user_id = ? OR lobby_id IN (SELECT lobbies.id FROM lobbies INNER JOIN recruits ON lobbies.recruit_id = recruits.id WHERE lobbies.owner_id = ? OR recruits.owner_id = ?) OR connection_id IN (SELECT id FROM connections WHERE user_a_id = ? OR user_b_id = ?)").bind(id,id,id,id,id),
    d1.prepare("DELETE FROM lobbies WHERE owner_id = ? OR recruit_id IN (SELECT id FROM recruits WHERE owner_id = ?)").bind(id,id),
    d1.prepare("DELETE FROM mutual_like_matches WHERE user_low_id = ? OR user_high_id = ? OR connection_id IN (SELECT id FROM connections WHERE user_a_id = ? OR user_b_id = ?)").bind(id,id,id,id),
