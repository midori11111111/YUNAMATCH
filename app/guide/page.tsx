...
export default function GuidePage() {
  return <main className={styles.page}><div className={styles.wrap}>
    ...
    <section className={styles.steps}>
      ...
      <article className={styles.step}><div><h2>候補に追加・プレイ申請</h2><p>「候補に追加」は、あとで一緒に遊びたいプレイヤーを整理する機能です。お互いが選ぶとチャットが開きます。「プレイ申請」は、今すぐ相談したい相手へ直接送る申請で、承認後にチャットが始まります。</p></div></article>
    ...
    <section className={styles.section}><small>PLAYER GUIDES</small><h2>第五人格の仲間探しガイド</h2><ul>{shoenmateGuides.map((guide) => <li key={guide.slug}><Link href={`/guide/${guide.slug}`}>{guide.title}</Link><p>{guide.description}</p></li>)}</ul></section>
