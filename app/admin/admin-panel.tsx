  ...
  return <main className="adminPage"><header><div><small>YUNAMATCH ADMIN</small><h1>運営ダッシュボード</h1></div><nav><Link href="/">アプリへ戻る</Link><button onClick={logout}>管理画面からログアウト</button></nav></header>
    <section className="adminAnalytics"><div className="adminSectionTitle"><div><small>ACCESS OVERVIEW</small><h2>アクセス状況</h2></div><button onClick={load}>更新</button></div>
      {loading||!stats?<p>集計中…</p>:<>
        ...
        <article className="adminGenderCard"><div><h3>登録ユーザーの男女比</h3><p>プロフィールに登録された性別で集計</p></div><div className="adminGenderNumbers"><span className="male"><b>男子 {stats.demographics.maleRate.toFixed(1)}%</b><small>{stats.demographics.male}人</small></span><span className="female"><b>女子 {stats.demographics.femaleRate.toFixed(1)}%</b><small>{stats.demographics.female}人</small></span></div><div className="adminGenderBar" aria-label={`男子${stats.demographics.maleRate.toFixed(1)}%、女子${stats.demographics.femaleRate.toFixed(1)}%`}><i style={{width:`${stats.demographics.maleRate}%`}}/><i style={{width:`${stats.demographics.femaleRate}%`}}/></div><small>合計 {stats.demographics.total}人</small></article>
