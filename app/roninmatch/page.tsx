...
import styles from "./roninmatch.module.css";
...
export default function RoninMatch({
  basePath = "/roninmatch",
}: {
  basePath?: string;
}) {
  const [auth, setAuth] = useState<
    ...
    [tab, setTab] = useState<Tab>(() => {
      ...
      const saved = window.localStorage.getItem("roninmatch:last-tab");
    ...
    [filters, setFilters] = useState<Filters>(() => {
      ...
      try {
        const stored = JSON.parse(
          window.localStorage.getItem("roninmatch:filters") ||
            '{"brawler":"","tier":""}',
  ...
  const load = async (nextFilters = filters) => {
    ...
    try {
      ...
      const discoverRequest = params.size
        ? fetch(`/api/services/roninmatch/discover?${params}`)
        : fetch("/api/services/roninmatch/discover");
      const [d, r, c] = await Promise.all([
          discoverRequest,
          fetch("/api/services/roninmatch/recruits"),
          fetch("/api/services/roninmatch/connections"),
        ]),
        likes = await fetch("/api/services/roninmatch/likes"),
  ...
  useEffect(() => {
    let live = true;
    fetch("/api/services/roninmatch/profile")
      .then(async (response) => {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem("roninmatch:last-tab", tab);
  }, [tab]);
  useEffect(() => {
    if (auth !== "ready" || typeof window === "undefined") return;
    if (!window.localStorage.getItem("roninmatch:tutorial-seen"))
  ...
  const current = profiles[0],
    ...
    completionItems = [
      ["受験生名", profile?.displayName],
      ["受験生タグ", profile?.gameIdentity],
      ["現在の模試判定", profile?.skillTier && profile.skillTier !== "未設定"],
      ["学習科目", currentBrawlers.length],
      ["勉強できる時間", profile?.playTimes?.length],
      ["自己紹介", profile?.bio],
  ...
  async function likeTarget(targetProfileId: number, after: () => void) {
    const response = await fetch("/api/services/roninmatch/likes", {
        method: "POST",
  ...
  async function requestTarget(targetProfileId: number) {
    const response = await fetch("/api/services/roninmatch/connections", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ targetProfileId }),
      }),
  ...
  async function createRecruit() {
    ...
    const response = await fetch("/api/services/roninmatch/recruits", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
    ...
    try {
      const controller = new AbortController(),
        ...
        response = await fetch(
          `/api/services/roninmatch/messages?connectionId=${connection.id}`,
  ...
  async function postMessage(
    ...
    const response = await fetch("/api/services/roninmatch/messages", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
  ...
  async function sendPlayInvite() {
    ...
    try {
      if (await postMessage("一緒に勉強しませんか？", "play_invite"))
        notify("一緒に勉強の申請を送りました");
    } finally {
  ...
  async function reactToMessage(item: Message, reaction: string) {
    ...
    try {
      const response = await fetch(
          "/api/services/roninmatch/message-reactions",
          {
            method: "PUT",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({
  ...
  function applyFilters() {
    window.localStorage.setItem("roninmatch:filters", JSON.stringify(filters));
    setFilterOpen(false);
    void load(filters);
  }
  function closeTutorial() {
    window.localStorage.setItem("roninmatch:tutorial-seen", "1");
    ...
    return (
      <article className={styles.card} key={`${received ? "received" : "recommended"}-${candidate.id}`}>
        ...
        >
          {candidate.avatarUrl ? (
            ...
            <>
              ...
              <strong>{candidateBrawlers[0] || "科目未設定"}</strong>
            </>
          )}
          <em>タップで詳しく見る</em>
        </button>
        <div className={styles.info}>
          ...
          <p>{candidate.bio || "同じ目標に向かって勉強できる仲間を探しています。"}</p>
    ...
    return (
      <main className={styles.login}>
        <section className={styles.intro}>
          <span className={styles.mark}>浪</span>
          <h1>RONIN MATCH</h1>
          <p>プロフィールを確認しています…</p>
        </section>
      </main>
  ...
  if (auth === "onboarding")
    return (
      <ServiceOnboarding
        ...
        identityLabel="志望校・学部"
        tiers={tiers}
        roles={brawlers}
        selectionLabel="得意・一緒に勉強したい科目（最大5つ）"
        selectionPicker
        selectionPlaceholder="科目を選ぶ"
        profileHeading="受験プロフィールを登録"
        tierLabel="現在の模試判定"
        timeLabel="勉強できる時間（複数可）"
        timeOptions={[
          "平日 朝（6〜9時）",
          "平日 昼（9〜17時）",
          "平日 夜（17〜22時）",
    ...
    return (
      <main className={styles.login}>
        <section className={styles.intro}>
          ...
          <p>
            志望校だけじゃない。現在の判定、科目、勉強時間から同じ一年を走る仲間を探そう。
          </p>
        ...
        <section className={styles.sheet}>
          ...
          <p className={styles.terms}>
            続けることで<a href="/legal?service=roninmatch">利用条件・安全方針</a>
  ...
  return (
    <main className={styles.app}>
      <header>
        ...
        <div>
          <span className={styles.headerMark}>浪</span>
          <b>
            RONIN MATCH<small>STUDY TOGETHER</small>
          </b>
        </div>
              ...
              <div className={styles.activeFilters}>
                ...
                <button
                  onClick={() => {
                    ...
                    setFilters(reset);
                    window.localStorage.removeItem("roninmatch:filters");
                    void load(reset);
                  }}
                >
                  解除
              ...
              recruits.map((item) => (
                ...
                >
                  ...
                  >
                    <span>{item.mode}</span>
                    <div>
                      <h2>{item.partySize}人の勉強会</h2>
                      <p>
                        {item.owner?.displayName || "退会ユーザー"} · 現在の判定 {item.owner?.skillTier}
                      </p>
                    </div>
                  ...
                  {expandedRecruitId === item.id && (
                    <div className={styles.recruitDetail}>
                      <p>
                        一緒に取り組む科目：{item.desiredRoles.join(" / ") || "指定なし"}
                        <br />
            ...
            {incoming.map((item) => (
              <article className={styles.pending} key={item.id}>
                ...
                <div>
                  ...
                  <p>
                    {item.other.skillTier} · {validBrawlers(item.other.roles).join(" / ") || "科目未設定"}
                  </p>
                  <button
        ...
        {tab === "me" && (
          <section className={styles.screen}>
            <div className={styles.profile}>
              ...
              <p>
                {profile?.skillTier} · {currentBrawlers.join(" / ") || "科目未設定"}
              </p>
            </div>
            ...
            <ServiceDiscordLink service="roninmatch" />
            <a
            ...
            <ServiceAccountSafety service="roninmatch" onNotice={notify} />
          </section>
        )}
      </section>
      <nav>
        ...
        <div className={styles.modalBackdrop}>
          <section className={`${styles.modal} ${styles.tutorial}`}>
            ...
            <div className={styles.tutorialList}>
              ...
              <article><b>⚡</b><div><strong>勉強仲間申請</strong><p>一緒に頑張りたい相手へ直接申請します。相手が承認するとチャットできます。</p></div></article>
              <article><b>×</b><div><strong>スキップ</strong><p>ブロックせず、今表示している候補を次の人へ送ります。</p></div></article>
              <article><b>＋</b><div><strong>募集</strong><p>今から勉強したい時は科目やスタイルを選び、参加者を募集できます。</p></div></article>
            </div>
      ...
      {recruitOpen && (
        <div className={styles.modalBackdrop}>
          <section className={styles.modal}>
            <div className={styles.modalHead}>
              <div><small>OPEN DESK</small><h2>勉強会を募集</h2></div>
            ...
            <div className={styles.formGrid}>
              ...
              <label>人数<select value={recruitForm.partySize} onChange={(event) => setRecruitForm((value) => ({ ...value, partySize: event.target.value }))}><option value="2">2人</option><option value="3">3人</option><option value="5">5人</option></select></label>
              <label>科目（任意）<select value={recruitForm.desiredBrawler} onChange={(event) => setRecruitForm((value) => ({ ...value, desiredBrawler: event.target.value }))}><option value="">指定なし</option>{brawlers.map((brawler) => <option key={brawler}>{brawler}</option>)}</select></label>
              ...
              <label className={styles.full}>ひとこと（任意）<textarea maxLength={160} value={recruitForm.note} onChange={(event) => setRecruitForm((value) => ({ ...value, note: event.target.value }))} placeholder="例：22時まで数学を進めます。無言自習でもOKです" /></label>
            </div>
      ...
      {viewProfile && (
        <div className={styles.modalBackdrop}>
          <section className={styles.modal}>
            ...
            <div className={styles.tags}>{validBrawlers(viewProfile.roles).length ? validBrawlers(viewProfile.roles).map((brawler) => <span key={brawler}>{brawler}</span>) : <span>科目未設定</span>}</div>
            <p className={styles.profileBio}>{viewProfile.bio || "自己紹介はまだありません。"}</p>
            <small className={styles.profileTimes}>{viewProfile.playTimes?.join(" · ") || "勉強できる時間は未設定です"}</small>
          </section>
        </div>
      )}
      {activeChat && (
        <div className={styles.chatOverlay}>
          ...
          <div className={styles.chatPrimaryActions}>
            ...
            >
              <b>机</b>
              <span>
                {playInviteSending ? "送信中…" : "一緒に勉強を申請"}
              </span>
            </button>
                ...
                return (
                  ...
                  >
                    ...
                    {item.response === "accepted" && (
                      <p className={styles.accepted}>✓ 一緒に勉強します</p>
                    )}
